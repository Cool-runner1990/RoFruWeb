...
export default function ScanDetailModal({ scan, onClose }: ScanDetailModalProps) {
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-4xl rounded-2xl border-2 border-primary/50 bg-surface shadow-[0_0_30px_rgba(59,130,246,0.4)] ring-2 ring-primary/20 flex flex-col max-h-[85vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 bg-surface border-b border-outline/20 rounded-t-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {/* Status Banner */}
          <div className={cn(
            "flex items-center gap-3 p-3 rounded-xl mb-5",
          ...
          {/* Zwei-Spalten Layout für Gerät und Artikel */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-2">
              ...
              <div className="p-4 rounded-xl bg-surface-variant/30 border border-outline/10 h-[calc(100%-2rem)]">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-xs text-on-surface-variant mb-0.5">Geräte-ID</p>
                    <p className="text-sm text-on-surface font-mono truncate">{scan.device_id}</p>
                  </div>
                  {scan.device_name && (
                    <div>
                      <p className="text-xs text-on-surface-variant mb-0.5">Gerätename</p>
                      <p className="text-sm text-on-surface">{scan.device_name}</p>
                    </div>
                  )}
                  <div className="col-span-2">
                    <p className="text-xs text-on-surface-variant mb-0.5">Scan-Zeitpunkt</p>
            ...
            <div className="space-y-2">
              ...
              <div className="p-4 rounded-xl bg-surface-variant/30 border border-outline/10 h-[calc(100%-2rem)]">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="text-xs text-on-surface-variant">GTIN</p>
                    <p className="text-sm font-mono font-semibold text-on-surface">{scan.gtin}</p>
                  </div>
                </div>
                ...
                {scan.article ? (
                  <div className="border-t border-outline/10 pt-3 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <p className="text-xs text-on-surface-variant">Artikelnummer</p>
                        <p className="text-sm font-mono text-primary font-semibold">
                          {scan.article.article_number}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-on-surface-variant">Bezeichnung</p>
                      ...
                      {scan.article.category && (
                        <div>
                          <p className="text-xs text-on-surface-variant">Bedarfsbereich</p>
                          <p className="text-sm text-on-surface">{scan.article.category}</p>
                        </div>
                      )}
                      {scan.article.branding && (
                        <div>
                          <p className="text-xs text-on-surface-variant">Branding</p>
          ...
          {/* Additional Data & Photo in einer Zeile */}
          {(scan.weight || scan.notes || scan.photo_url) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
              ...
              {(scan.weight || scan.notes) && (
                <div className="space-y-2">
                  ...
                  <div className="p-4 rounded-xl bg-surface-variant/30 border border-outline/10 space-y-3">
                    {scan.weight && (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-semibold text-on-surface">{scan.weight} kg</p>
                        </div>
                      </div>
                    )}
                    {scan.notes && (
                      <div>
                        ...
                        <p className="text-sm text-on-surface bg-surface/50 p-2 rounded-lg">
                          {scan.notes}
                        </p>
                      </div>
                    )}
              ...
              {scan.photo_url && (
                <div className="space-y-2">
                  ...
                  <div className="rounded-xl overflow-hidden border border-outline/20 max-h-48">
                    <img
                      src={scan.photo_url}
                      alt="Scan-Foto"
        ...
        {/* Footer */}
        <div className="flex-shrink-0 px-6 py-4 bg-surface border-t border-outline/20 rounded-b-2xl">
          <button
            onClick={onClose}
